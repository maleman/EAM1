...
Ichimoku::lookforsigns(){
   ...
   if(tenkanSenBuffer[0] < kijunSenBuffer[0] 
      && tenkanSenBuffer[tenkanSize] > kijunSenBuffer[kijunSize]){//uptrend cross finded
      ...
      if(tenkanSenBuffer[tenkanSize] > senkouSpanABuffer[sekouSize]  
         ...
         && kijunSenBuffer[kijunSize] > senkouSpanBBuffer[sekouSize]){ // KUMO over 
   ...
   if(tenkanSenBuffer[0] > kijunSenBuffer[0] 
      && tenkanSenBuffer[tenkanSize] < kijunSenBuffer[kijunSize]){//downtrend cross finded
   ...
   if(senkouSpanABuffer[0] < senkouSpanBBuffer[0] 
      && senkouSpanABuffer[sekouSize] > senkouSpanBBuffer[sekouSize]){// uptrend cross
   ...
   if(rates[rateSize].open < kijunSenBuffer[kijunSize] && rates[rateSize].close > kijunSenBuffer[kijunSize]){ // UPTREND CROSS
